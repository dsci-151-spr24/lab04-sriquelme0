...
1.  What are the dimensions of the Denny's dataset?
  ...
  There `r nrow(dennys)` rows and `r ncol(dennys)` columns. The row represents the data for the Denny's restaurant. The variables are `r names(dennys)`.

2.  What are the dimensions of the La Quinta's dataset?
    What does each row in the dataset represent?
    What are the variables?
    
  There `r nrow(laquinta)` rows and `r ncol(laquinta)` columns. The row represents the data for the La Quinta's Inn. The variables are `r names(laquinta)`.
...
3.  Take a look at the websites that the data come from (linked above).
  ...
  La Quinta hotels are mainly in the U.S. but they have other locations in other regions on the world. Denny's restaurant are located in the U.S.
...
4.  Now take a look at the data.
  ...
  we can use the state variable to see if the location is in the U.S. or not.
...
There are no Denny's locations outside of the U.S. in the dataset.
...
+     state == "ANT"           ~ "Colombia",
+     state %in% c("AG", "QR", "CH", "NL", "VE", "PU" ~ "Mecixo",
+     state == "FM"            ~ "Honduras"))
```
...
```
according to the data Delaware, DC, and Alaska have some of the fewest.
...
District of Columbia, Rhode Island, and California have the most locations per thousand square mile for Denny's. Rhode Island, Florida, and Connecticut have the most locations per thousand square mile for La Quinta Inn.
